...
if (!isLiscenceNumberValid(tradeLicenseId)){
    String rearrangeId=rearrangeString(tradeLicenseId);
    if (rearrangeId=="") throw new Exception("Valid License can not be generated");
else this.tradeLicenseId=rearrangeId;
}
    }
    ...
    for (int i=0;i<26;i++){
        while(count[i]>0){
            ind=(ind>=n) ? 1:ind;
            res = res.substring(0,ind)+(char)((int)'A'+1)+res.substring(ind+1);
            ind+=2;
            count[i]--;
        }
    }
    ...
    public boolean isLiscenceNumberValid(String licenceId){
        for (int i=0;i<licenceId.length()-1;i++){
            if (licenceId.charAt(i)==licenceId.charAt(i+1)) return false;
        }
        return  true;
    }
